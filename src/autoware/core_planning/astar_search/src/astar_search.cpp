...
bool AstarSearch::search() {
    ...
    // 开始 A* 搜索
    // 如果 openlist 为空，则搜索失败
    while (!openlist_.empty()) {
        // 检查时间，如果搜索达到时间限制，则终止搜索
        ...
        for (const auto &state : state_update_table_[top_sn.index_theta]) {
            ...
            if (use_potential_heuristic_) {
                ...
                next_hc +=
                    calcDistance(next_x, next_y, goal_pose_local_.pose.position.x, goal_pose_local_.pose.position.y) *
                    distance_heuristic_weight_;
            }
