...
AstarAvoid::AstarAvoid()
    : nh_(), private_nh_("~"), closest_waypoint_index_(-1), obstacle_waypoint_index_(-1), closest_local_index_(-1),
      costmap_initialized_(false), current_pose_initialized_(false), current_velocity_initialized_(false),
      base_waypoints_initialized_(false), closest_waypoint_initialized_(false), terminate_thread_(false) {
...
AstarAvoid::~AstarAvoid() { publish_thread_.join(); }
...
void AstarAvoid::obstacleWaypointCallback(const std_msgs::Int32 &msg) { obstacle_waypoint_index_ = msg.data; }
...
void AstarAvoid::run() {
    ...
    while (ros::ok()) {
        ...
        if (state_ == AstarAvoid::STATE::RELAYING) {
            ...
            if (planAvoidWaypoints(end_of_avoid_index)) { // 正式开始规划
                // 规划成功
            ...
            bool reached = (getLocalClosestWaypoint(avoid_waypoints_, current_pose_global_.pose, closest_search_size_) >
                            end_of_avoid_index);
            if (reached) {
...
/**
 * @brief 规划避障路径
 * @param end_of_avoid_index 避障路径的结束索引, goal 所在的 id
 * @return 是否找到路径
 */
bool AstarAvoid::planAvoidWaypoints(int &end_of_avoid_index) {
    bool found_path = false;
    int closest_waypoint_index =
        getLocalClosestWaypoint(avoid_waypoints_, current_pose_global_.pose, closest_search_size_);
...
/**
 * @brief 将 A* 算法规划出的路径合并到原始的避障路径中
 * @param path A* 算法规划出的路径
 * @param end_of_avoid_index 避障路径的结束索引
 */
void AstarAvoid::mergeAvoidWaypoints(const nav_msgs::Path &path, int &end_of_avoid_index) {
    ...
    // 获取当前位置在避障路径中的最近点索引
    int closest_waypoint_index =
        getLocalClosestWaypoint(current_waypoints, current_pose_global_.pose, closest_search_size_);
    ...
    for (const auto &pose : path.poses) {
        ...
        // 设置路径点的高度为当前位置的高度
        wp.pose.pose.position.z = current_pose_global_.pose.position.z;
        // 设置路径点的速度为避障速度
